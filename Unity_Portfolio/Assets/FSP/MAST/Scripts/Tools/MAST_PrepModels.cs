...
#region StripMaterials
    ...
    public bool CreatePrefabs(string targetPath, List<Material> sourceMat, List<string> sourceMatName,
                              ...
                              bool flagAddMeshCollider, bool flagAddEmptyParent, bool flagPreserveModelHierarchy)
    {
        ...
        for (int i = 0; i < model.Length; i++)
        {
            ...
            if (flagAddEmptyParent)
            {
                ...
                // If preserving model hierarchy
                if (flagPreserveModelHierarchy)
                {
                    prefabGameObjectChild = CreateGameObjectFromModelWithPreservedHierarchy(model[i].transform, searchMatName, sourceMat,
                        targetPath, flagAddMeshCollider, model[i].name, 1, 1);
                }
                
                // If combining all meshes in model
                else
                {
                    // Create and get reference to a new GameObject from the model by using a recursive method
                    prefabGameObjectChild = CreateGameObjectFromModelCombined(model[i].transform, searchMatName, sourceMat,
                        targetPath, flagAddMeshCollider);
                    
                    prefabGameObjectChild.name = "Mesh";
                }
            ...
            else
            {
                ...
                if (flagPreserveModelHierarchy)
                {
                    prefabGameObject = CreateGameObjectFromModelWithPreservedHierarchy(model[i].transform, searchMatName, sourceMat,
                        targetPath, flagAddMeshCollider, model[i].name, 1, 1);
                }
                ...
                {
                    // Create and get reference to a new GameObject from the model by using a recursive method
                    prefabGameObject = CreateGameObjectFromModelCombined(model[i].transform, searchMatName, sourceMat,
                        targetPath, flagAddMeshCollider);
                    
                    prefabGameObject.name = model[i].name;
                }
    ...
    private GameObject CreateGameObjectFromModelCombined(Transform modelTransform, List<string> searchMatName,
                                                        List<Material> savedMat, string targetPath, bool flagAddMeshCollider)
    {
        ...
        GameObject newGameObject = new GameObject(modelTransform.gameObject.name);
        
        // Get all MeshFilters and MeshRenderers from the source GameObject, including all children
        MeshFilter[] modelMeshFilters = modelTransform.gameObject.GetComponentsInChildren<MeshFilter>();
        MeshRenderer[] modelMeshRenderers = modelTransform.gameObject.GetComponentsInChildren<MeshRenderer>();
        
        // Create a List containing all unique Materials found in the Model
        List<Material> uniqueMats = MergeMeshes.GetUniqueMaterialListFromMeshRendererArray(modelMeshRenderers);
        
        // Create a GameObject with all SubMeshes combined into a single Mesh
        GameObject combineMeshGameObject = MergeMeshes.CombineMeshesWithSubMeshes(uniqueMats, modelMeshFilters, modelMeshRenderers);
        
        // Create mesh path + filename
        string saveMeshPath = targetPath + "/Meshes/mesh_" + modelTransform.gameObject.name + ".asset";
        
        // Create the Mesh Object
        Mesh saveMesh = (Mesh)GameObject.Instantiate(combineMeshGameObject.GetComponent<MeshFilter>().sharedMesh);
        ...
        // Add MeshFilter component to the GameObject and reference the saved Mesh
    ...
    private GameObject CreateGameObjectFromModelWithPreservedHierarchy(Transform modelTransform, List<string> searchMatName,
                                                 List<Material> savedMat, string targetPath, bool flagAddMeshCollider,
                                                 string saveName, int saveLevel, int saveIndex)
    {
        ...
        if (modelMeshRenderer)
        {
            ...
            string saveMeshPath = targetPath + "/Meshes/mesh_" + saveName + "_lvl" + saveLevel + "_ndx" + saveIndex + ".asset";
        ...
        for (int i = 0; i < modelTransform.childCount; i++)
        {
            ...
            newChildGameObject = CreateGameObjectFromModelWithPreservedHierarchy(modelTransform.GetChild(i), searchMatName, savedMat,
