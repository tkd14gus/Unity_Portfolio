...
public class MAST_Prefab_Creator_Window : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        switch (processStep)
        {
            ...
            case 1:  // Strip Materials Section
                GUIFindMaterials();
                break;
                
            case 2:  // Organize Materials Section
                GUIOrganizeMaterials();
                break;
                
            case 3:  // Link Materials Section
                GUILinkMaterials();
                break;
                
            case 4:  // Create Prefabs Section
                GUICreatePrefabs();
                break;
        }
    ...
    private void GUIToolBar()
    {
        ...
        GUI.enabled = (processStep > 0);
        if (GUILayout.Button(new GUIContent("Step 2:  Get Materials",  "Return to Step 2")))
            processStep = 1;
        
        GUILayout.Space(5f);
            
        GUI.enabled = (processStep > 1);
        if (GUILayout.Button(new GUIContent("Step 3:  Organize Materials", "Return to Step 3")))
            processStep = 2;
        
        GUILayout.Space(5f);
        
        GUI.enabled = (processStep > 2);
        if (GUILayout.Button(new GUIContent("Step 4:  Link Materials", "Return to Step 4")))
            processStep = 3;
        
        GUILayout.Space(5f);
        
        GUI.enabled = (processStep > 3);
        if (GUILayout.Button(new GUIContent("Step 5:  Create Prefabs", "")))
            processStep = 4;
    ...
    private void GUIFindMaterials()
    {
        ...
        GUILayout.BeginVertical("MAST Toolbar BG Inset");
        GUILayout.Space(5f);
        EditorGUILayout.LabelField("Step 2:  Find Materials", EditorStyles.boldLabel);
        EditorGUILayout.LabelField("Get a unique list of materials gathered from models");
        GUILayout.Space(5f);
        EditorGUILayout.EndVertical();
        ...
        if (GUILayout.Button(new GUIContent("Find Materials in Models",
            "Create a unique list containing all materials found on models in the selected folder.")))
        {
        ...
        // -------------------------------------
        // List - Source Material Name
        // -------------------------------------
        
        GUILayout.BeginVertical("MAST Toolbar BG Inset");
        GUILayout.Space(5f);
        
        EditorGUILayout.LabelField("Materials Found");
        
        if (sourceMat != null)
        {
            findMatScrollPos = EditorGUILayout.BeginScrollView(findMatScrollPos);
            
            // Display a Selection grid of all Source Material Names
            GUILayout.SelectionGrid(-1, sourceMatName, 1);
        ...
        // Disable GUI for the "Done with Finding Materials" button
        GUI.enabled = (sourceMat != null);
        
        // Done with Finding Materials button
        if (GUILayout.Button(new GUIContent("Done with Finding Materials", "Go to next step")))
        {
        ...
        // Enable the GUI if it was disabled
    ...
    private void GUICreatePrefabs()
    {
        ...
        if (GUILayout.Button(new GUIContent("Create Prefabs", "Create prefabs")))
        {
            PrepModels.CreatePrefabs(modelPath, sourceMat, new List<string>(sourceMatName),
                                     ...
                                     flagAddMeshCollider, flagAddEmptyParent, flagPreserveModelHierarchy);
        }
