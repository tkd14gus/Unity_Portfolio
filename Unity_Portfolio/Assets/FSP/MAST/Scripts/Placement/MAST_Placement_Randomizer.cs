...
public static class MAST_Placement_Randomizer
{
    ...
    private static void GetRandomPrefabInCategory(int categoryID)
    {
        ...
        for (int prefabIndex = 0; prefabIndex < MAST_Prefab_Palette.GetPrefabArray().Length; prefabIndex++)
        {
            // Get the MAST component script attached to this prefab
            mastScript = MAST_Prefab_Palette.GetPrefabArray()[prefabIndex].GetComponent<MAST_Prefab_Component>();
        ...
        MAST_Prefab_Palette.selectedItemIndex = replacementPrefabIndexList[replacementPrefabIndex];
