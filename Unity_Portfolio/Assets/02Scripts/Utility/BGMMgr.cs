...
public class BGMMgr : MonoBehaviour
{
    ...
    Dictionary<string, AudioClip> bgmTable; //BGM파일들을 담아놓을 딕셔녀리 (STL Map)

    AudioSource audioMain;                  //메인오디오
    AudioSource audioSub;                   //서브오디오(BGM 교체시 사용)

    [Range(0, 1.0f)]                        //[]로 만들어져 있는 Attribute라고 하고 인스펙터창 값을 0 ~1로 고정
    public float masterVolume = 1.0f;       //마스터 볼륨은 0 ~ 1.0f
    ...
    void Update()
    {
        //브금이 플레이중일 때 메인볼륨은 올리고 서브 볼륨은 내린다.
        ...
        audioSub.volume = volumeMain * masterVolume;
    }

    //BGM플레이
    public void PlayBGM(string bgmName)
    {
        //딕셔너리 안에 브금이 없으면 리소스 폴터에서 찾아서 새로 추가하자.
        if(bgmTable.ContainsKey(bgmName) == false)
        {
            ...
            //리소스폴더에 bgm이 없다면 딕셔너리에 추가하지 말고 나온다.
            //오디오파일이 없으니 재생할 수 없다.
            if (bgm == null) return;

            //딕셔너리에 bgmName의 키값으로 bgm릏 추가하자
