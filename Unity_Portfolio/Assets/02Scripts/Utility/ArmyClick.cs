...
public class ArmyClick : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //BattleUI가 있다면(맵 구경이 아닌 전투하는 중이라면)
        //바로 꺼버린다.
        if (battleUI != null)
        {
        ...
        {
            ArmyManager.instance.gameObject.SetActive(false);
            ArrowManager.instance.gameObject.SetActive(false);
            GameObject.Find("ArmySpawnManager").SetActive(false);
            GameObject.Find("ShipManager").SetActive(false);
            isBattle = false;
        }
    }
